...
def main_loop(_):
    ...
    # =================================================
        ...
        if abs(dx) + abs(dy) > diag_thr:
            if dx > 300 and dy > 300:   move_num = 6   # 左下
            if dx < -300 and dy > 300:   move_num = 7   # 右下
            if dx > 300 and dy < -300:   move_num = 4   # 左上
            if dx < -300 and dy < -300:   move_num = 5   # 右上
            if dx > 1000 and dy < 300 and dy > -300: move_num = 2 # 左
            if dx < -1000 and dy < 300 and dy > -300: move_num = 3 # 右
            if dx >= -300 and dx <= 300 and dy < -1000: move_num = 0 #上
            if dx >= -300 and dx <= 300 and dy > 1000: move_num = 1 #下
        else:
            move_num = 8                             # 停止
